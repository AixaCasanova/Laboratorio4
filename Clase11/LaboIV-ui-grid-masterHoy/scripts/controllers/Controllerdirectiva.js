angular
  .module('app')
  .controller('Controllerdirectiva', function($scope, data, i18nService, uiGridConstants, factoryCServBandera) {
    $scope.titulo = "Control directiva";
    $scope.listadoBanderas={};

     factoryCServBandera.TraerTodos().then(function(resp)
            {
                $scope.listadoBanderas=resp;
                console.info(resp);
            });

    // $scope.nombrepais="argentina";
    // $scope.fotobandera="untBandera.jpg";
